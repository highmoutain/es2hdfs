...
import org.apache.spark.sql.{CarbonSession, SaveMode, SparkSession, CarbonEnv}
...
//    val spark = SparkSession.setActiveSession(sparkSession)
//
//    SparkSession carbonSession = CarbonSession.CarbonBuilder(
//      SparkSession.builder().spar
//        //config(stringJavaRDD.context().getConf()).
//        config("spark.sql.warehouse.dir", warehouse)
//    ).getOrCreateCarbonSession(warehouse,metastore)


    //CarbonSession.CarbonBuilder(sparkSession.)

    val spark = SparkSession
      .builder()
      .appName("StreamExample")
      .config("spark.sql.warehouse.dir", warehouse)
      //.config("userSuppliedContext",sparkSession.sparkContext)
      .getOrCreateCarbonSession(warehouse, metastore)

    //Thread.sleep(1000000)

    val df2 = spark.sqlContext.read.options(Map("kudu.master" -> "172.20.40.18:7051,172.20.40.20:7051,172.20.40.21:7051", "kudu.table" -> kuduTableName)).kudu
    //改为注册为临时表，用sql语句试
    //df2.registerTempTable("temp")
    df2.createOrReplaceTempView("profile_kudu")
    ...
    //val dfoutput = spark.sql("insert overwrite table behavior  select  *   from temp")
    spark.sql(sql).show()
...
//      .save()
  ...
  JobData Or Every[ValidationProblem] = {
    Try(List(config.getString("kuduTableName"), config.getString("sql")))
